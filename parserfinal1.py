...
def set_dir():
    dir_images = "TEI/images/" 
    dir_notes = "TEI/new_volumes/notebooks_47-58/"
    base_dir = os.getcwd()
    path2images = os.path.join(base_dir, dir_images)
    path2notebooks = os.path.join(base_dir, dir_notes)
    return path2notebooks, path2images
def set_df():
    base_dir = os.getcwd()
    notebooks = "notebooks_table.csv"
    path2notebooks = os.path.join(base_dir, notebooks)
    df = pd.read_csv(path2notebooks)
    df["files"] = ""
    return df
...
parts_dic = {"календарныйблокнотзаписисянваряпоапрель1902г":}
...
def entry_line_tag(tag_, text_tag):
    pattern1 = re.compile("1[0-9]{3}—1[0-9]{3}\s*\?{0,1}\s*гг")
    pattern2 = re.compile("1[0-9]{3}\s*\?{0,1}\s*г")
    global date2parser 
    if tag_.tag == "p":
        ...
        if "h" in tag_.tag:
            if isinstance(tag_.text, str):
                if pattern1.search(tag_.text):
                   ...
                   # print(date2parser, tag_.text)
                elif pattern2.search(tag_.text):
                    date2parser = pattern2.search(tag_.text).group()
                    #print(date2parser, tag_.text)
...
def change2tei(root):
    ...
    for tag_ in text.iter():
        ...
        elif tag_.tag == "div" and tag_.get("class") == "image":
            tag_.tag = "figure"
            del tag_.attrib["class"]
...
def pipline():
    ...
    for vol_id in volume_id:
      ...
      for file in html_dict:
          title = " ".join([a.text if isinstance(a,  bs4.element.Tag) else a for a in get_title(soup, file)])
          ...
          clean_title = re.sub("[^а-я\d]*","",title.lower())
          if clean_title == "записнаякнижка518931895гг":
              clean_title = "записнаякнижка518931894гг"
          df2 = df[df["value"] == vol_id] 
          check = df2["check"].str.contains(clean_title)
          print(clean_title, "!!!!", get_title(soup, file))
          if df2[check].shape[0] == 1:
              #print("contains: ", clean_title)
              title = df2[check].iloc[0,9]
              file2.find(".//title").text =  title
              df.loc[df['имя из 90-томника'] == title, df.columns[21]] = f"{title} {vol_id}.xml"

              
          else:
              print("doesn't contain ", clean_title)
              for head in file2.findall(".//head"):
                  print(etree.tostring(head, pretty_print=True, encoding = "unicode"), "head!")
                  # title2 = " ".join([a.text if isinstance(a,  bs4.element.Tag) else a for a in head])
                  

                  # clean_title = re.sub("[^а-я\d]*","",title2.lower())

                  # check = df2["check"].str.contains(clean_title)
                  
                  # if df2[check].shape[0] == 1:
                  #     df.loc[df['имя из 90-томника'] == title2, df.columns[21]] = f"{title2} {vol_id}.xml"
